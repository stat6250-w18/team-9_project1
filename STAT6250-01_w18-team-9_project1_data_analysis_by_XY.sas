...
title1
'Research Question: What is the mean value for unempolyment and median household income in the US in 2015?'
;
...
footnote3
'The two statistics will be an indicator for the following data analysis steps, it is meaningful to look at into states statistics.'
;

*
Methodology: Use PROC MEANS to get the descpriptive stats of all the observations from
the temporary dataset created in the corresponding data-prep file.

Limitations: This methodology does not account for districts with missing data,such as Purto Rico area, and there is a huge gap 
between areas in the US, so the average lack of some credibility as an indicator.

Possible Follow-up Steps: Carefully review the variables for house median income and unemployment rate, clean up the missing data 
and maybe use an average rate as a proxy for increasing the accuracy.
;
...
title1
'Research Question: What are the top 10 states with the highest mean values of unemployment rate in 2015?'
;
...
footnote2
'When we compare the state median household income with the top ten highest unemployment rate states, we found all the states, execpt for PR and AK, are
...
*
Methodology: Use proc means to study the unemployment rate and median household income of state,with proc sort to get first 10 observations with highest unemployment rate.
.

Limitations: The missing data is not well treated, such as counties in AK. Those will underestimate/overestimate the average statewide rate.

Follow-up Steps: A possible follow-up to this approach could use the national average rate as a proxy to replace the missing data.
;
proc means
        mean
        noprint
...
title1
'Research Question: Can "Unemployment Rate" be used to predict "Median household income"?'
;
...
footnote1
"Based on the above output, there's no clear inferential pattern for predicting the unemployment rate from the median household income since cell counts don't tend to follow trends for increasing or decreasing consistently."
;
...
;
*
Methodology: Use Proc format to create formats to bin values of Median_Household_2015_Income and Unemployment_2015_rate
...
statistical technique like beta regression, or use linear regression models to find the relationship.
;
